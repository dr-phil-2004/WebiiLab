...
import { ValidationErrors, ValidationResult, validateAdditionalInfo, validateBasicInfo, validateCTA } from "@/lib/type";
...
type WebinarStore = {
    ...
    updateBasicInfoField:< K extends keyof WebinarFormState['basicInfo'] > (field : K, value : WebinarFormState['basicInfo'][K])=> void
...
export const useWebinarStore = create<WebinarStore>((set, get)=>({
    ...
    validateStep: (stepId: keyof WebinarFormState) => {
        ...
        set((state) => ({
            validation : {...state.validation, [stepId]: 
                validationResult}
        }))
